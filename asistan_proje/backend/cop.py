...
gemini_index = faiss.IndexFlatIP(384)
openai_index = faiss.IndexFlatIP(384)
gemini_chunks = []
openai_chunks = []
...
def search_faiss(query, index):
    query_embedding = model.encode([query]).astype('float32').reshape(1, -1) #.reshape(1, -1): model.encode tek bir sorgu için genellikle 1 boyutlu bir dizi döndürür. 
    # query_embedding burada 1 tane 384 boyutlu vektörü temsil eden bir matris haline gelir.
    D, I = index.search(query_embedding, k=6) #Distance 
    #Indices
    return I[0]
...
# Benzerlik dağılımını kontrol

# Asistan Fonksiyonları
...
# API Endpoint'leri
@app.route("/ask/anythingllm", methods=["POST"])
def anythingllm_chat():
    user_message = request.json.get("query")

    url = "http://10.60.0.166:80/api/v1/workspace/aichat/chat"
    headers = {
        "Authorization": "Bearer KPV2K5X-M6AMW7V-MWMTM9X-N16ABC7",
    ...
    try:
        ...
        # Sadece `textResponse` alanını al
        result = response.json().get("textResponse", "Cevap bulunamadı.")
...
if __name__ == "__main__":
    try:
        ...
        # Uygulama kapanırken observer'ı durdur
